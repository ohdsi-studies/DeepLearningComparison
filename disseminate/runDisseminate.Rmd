...
p <- ggplot(complete_data, aes(x = validation_database_meta_data_id, y = database_meta_data_id, fill = value)) +
  ...
  scale_y_discrete(expand = c(0, 0)) +
  scale_x_discrete(expand = c(0, 0)) +
  scale_fill_viridis(name=expression(E[avg]), limits = c(0.0, 0.08), option = "D", na.value = "#F0F0F0") +
  labs(title = NULL,
...
process_file <- function(file_path) {
  ...
  number_O <- extract_number(filename_without_ext)

  # If no match is found for model, database, or number, return NULL
  if (is.na(model) || is.na(database) || is.na(number_O)) {
    return(NULL)
  }
  ...
  data <- data %>%
    mutate(model = model, database = database, number_O = number_O)
...
```{r}
  ...
  if(metric == "AUROC") {
    ...
    tidyLearningCurve <- learningCurve %>% 
      ...
      dplyr::select("trainFraction", "Occurrences", "Observations", "Test_AUROC", "Train_AUROC", "model", "database", "number_O")
    
    exclude_columns <- c("model", "database", "number_O")
    ...
    tidyLearningCurve <- tidyr::pivot_longer(
      ...
      cols = colnames(as.data.frame(tidyLearningCurve))[!colnames(as.data.frame(tidyLearningCurve)) %in% c('trainFraction', 'Occurrences', 'Observations', "model", 'database', "number_O")], 
      values_to = "value", 
      names_to = 'variable'
    )
    ...
    tidyLearningCurve <- learningCurve %>% 
      dplyr::rename(
        Occurrences = "Train_outcomeCount", 
        Observations = "Train_populationSize" ) %>%
      dplyr::select("trainFraction", "Occurrences", "Observations", "Test_AUPRC", "Train_AUPRC", "model", "database")
  ...
  tidyLearningCurve <- tidyLearningCurve %>%
    dplyr::filter(database == "cdm_optum_ehr")

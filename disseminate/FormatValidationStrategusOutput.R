...
getAllAucValidation <- function(strategusOutputPath) {
  ...
  for (dir in subdirs) {
    ...
    for(performancesFilePath in allFiles) {
      ...
      evaluationStatisticData <- evaluationStatisticData[evaluationStatisticData$metric == "AUROC" | evaluationStatisticData$metric == "95% lower AUROC" | evaluationStatisticData$metric == "95% upper AUROC" | evaluationStatisticData$metric == "AUPRC" | evaluationStatisticData$metric == "Eavg", ] %>%
  ...
  finalSelectedData <- combinedData %>%
    select(database_meta_data_id, validation_database_meta_data_id, model_design_id, model_type, AUROC, `95% lower AUROC`, `95% upper AUROC`, AUPRC, Eavg, cohort_definition_id, cohort_name) %>%
